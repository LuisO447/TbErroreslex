...
namespace MiniCompiladorXconsola
{
    class Program
    {
        ...
        "SINO", "FIN", "VERDADERO", "FALSO" };      //las unicas palabras reservadas dentro del lenguaje  

        private static bool ValidacionAZ(String str)
        {
            return Regex.IsMatch(str, @"^[a-z]+$");//Expresion regular para la 'a' a la 'z'.
        }

        private static bool ValidacionNUM(String str)
        {
            return Regex.IsMatch(str, @"^[0-9]+$");//expresion regular para los digitos del 0 al 9.
        }

        private static bool Min(String str)
        {
            bool tieneMinusculas = str.Any(c => char.IsLower(c)); //verificar si tiene o no minusculas.
            return tieneMinusculas;
        }

        private static bool May(String str)
        {
            bool tieneMayusculas = str.Any(c => char.IsUpper(c)); //verficar si tiene o no mayusculas.
            return tieneMayusculas;
        }

        private static bool Dig(String str)
        {
            bool tieneNumeros = str.Any(c => char.IsDigit(c)); //ver si la cadena ingresada contiene digitos. 
            return tieneNumeros;
        }
        ...
        static void Verificacion(string [] x)
        {
            ...
            //este case funciona como la tabla de errores
            switch (variable)
            {
                case "ENTERO":
                    ...
                    else if (ValidacionNUM(dato) == true && !ultimoCaracter.Equals(";")) //si no puso ; al final de la cadena
                    {
                        if (!ultimoCaracter.Equals(";") && ValidacionAZ(ultimoCaracter) == true || !ultimoCaracter.Equals(";") && Dig(ultimoCaracter) == true)
                        {
                            texto = "IDENTIFICADOR..............\t\t\t" + dato + "\tIDENTIFICADOR NO VALIDO\n" +
                    ...
                    break;
                case "DECIMAL": //case para cada palabra reservada, en este caso para DECIMAL
        ...
        static void Main(string[] args)
        {
            ...
            Console.WriteLine("\t\t\t\t\t////////////////////////////////////////////////////////////////////////////////////////");
            Console.WriteLine("\t\t\t\t\t----------------------------------------------------------------------------------------");
            Console.WriteLine("\t\t\t\t\t-----------------------------Compilador Sin terminar, tb de errores-----------------------------------");
            Console.WriteLine("\t\t\t\t\t----------------------------------------------------------------------------------------");
            Console.WriteLine("\t\t\t\t\t////////////////////////////////////////////////////////////////////////////////////////");
            Console.WriteLine("\t\t\t\t\t----------------------------------------------------------------------------------------");
            Console.WriteLine("\t\t\t\t\t||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||");
